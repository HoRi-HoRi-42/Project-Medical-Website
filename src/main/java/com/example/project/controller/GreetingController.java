...
import com.example.project.model.HelloMessage;
	...
	public Greeting greeting(HelloMessage message) throws Exception {
		...
		try {
			...
			if (textLine.equals("q")) {
                ...
                /*
				if(requestmsg.equalsIgnoreCase("buna") || requestmsg.equalsIgnoreCase("salut"))
					response = "Buna ziua! Numele meu este Mihai! Cu ce va pot ajuta astazi?";
				if(requestmsg.contains("rau"))
					response = "Puteti sa-mi dati mai multe detalii?";
				if(requestmsg.contains("cap"))
					response = "In ce zona va doare mai exact?";
				if(requestmsg.contains("frunte") || requestmsg.contains("fruntii"))
					response = "S-ar putea sa aveti probleme cu sinusurile. Mai sesizati si alte probleme?";
				if(requestmsg.contains("nas") || requestmsg.contains("gat") || requestmsg.contains("urechi"))
					response = "Va voi trimite sa discutati cu un medic - sectia ORL";
				if(requestmsg.contains("ceafa") || requestmsg.contains("in spate") || requestmsg.contains("cefii"))
					response = "Va voi trimite sa discutati cu un medic - sectia neurologie";
                else
                 */
                ...
                if(response.contains("redirectiona")) {
					response.concat(" http://localhost:8080/chat");
				}
